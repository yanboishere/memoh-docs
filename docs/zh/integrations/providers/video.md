...
Bot 靠 `generate_video` 工具生成视频。要用上它，两步缺一不可：

1. 在 **设置 → 集成 → Video** 里配好**视频提供方**和至少一个视频模型；
2. 在 **Bot 设置 → 多媒体** 里给 Bot 选一个**视频生成模型**。

没选视频模型的 Bot 根本不会拿到 `generate_video` 这个工具。
...
5. 去 Bot **设置 → 多媒体**，选上**视频生成模型**。

---

## Bot 怎么用

直接跟 Bot 说"给我生成一个 xx 的视频"就行。背后发生的事：

- `generate_video` 是**后台任务**，立刻返回 task ID；生成通常要几分钟，Bot 自己用后台任务工具盯进度。
- 参数除了 `prompt`，还有可选的 `duration_seconds`、`resolution`（`720p`/`1080p`）、`aspect_ratio`（`16:9`、`9:16`、`1:1`）、提供方专属的 `size`、`generate_audio`。
- 成片下载到 Bot workspace 的 `/data/generated-videos/<task_id>.mp4`（也可能是 `.webm`/`.mov`，看提供方输出）。
- 视频**不会自动发出去**——Bot 以消息附件形式交付，你也可以去 workspace 的 **Files** tab 拿。

...
固定命令之外，斜杠还能按需激活 Bot 的[技能](./skills.md)：

```text
/<技能名> [可选的提示词]
```

- **网页 composer** 里输入 `/` 会弹面板：**快捷操作**（`/help`、`/skill list`、`/new`、`/compact`、`/model`）加上 Bot 的技能列表。选中的技能变成可移除的 chip，也可以直接打上面那种形式。
- **IM 渠道**里同样支持 `/<技能名>`；`/skill list` 会渲染成可点的按钮。
- 激活的技能内容**只注入当前这一轮**——不进历史、不进会话标题、不进记忆。

技能名每次发送都在**服务端**按 Bot 当前生效的技能目录解析。长得像命令/技能但都对不上的斜杠输入会明确报错，不会漏给模型；只是开头带 `/` 的普通文本（路径、URL）照常当聊天。

两个限制：

- 技能激活消息不能带附件（固定命令带附件照常执行）。
- 群聊里没有指向 Bot 的斜杠消息会被静默忽略。
...
| `/link <code>` | 用客户端里生成的码，把当前渠道账号绑定到 Memoh 用户 | `consume` |
...
| `latest` | 若当前路由没有活跃会话，要看**该 Bot 最新**会话时用 |
...
要按轮激活技能，用 `/<skill-name> [提示词]`，见上面的 [技能激活与快捷操作](#技能激活与快捷操作)。

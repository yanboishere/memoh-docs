...
- **渠道成员**：控制来自 Telegram、Discord、飞书、QQ 等 IM 渠道的身份。
- **工作区成员**：控制客户端和工作区里的 Memoh 注册用户。
...
| **工作区成员** | Memoh 注册用户账号 | 这个用户能否在客户端使用 Bot、读写文件、执行工作区命令或管理 Bot 设置 |
...
每一行代表一个**渠道身份**（一个 Telegram 用户或 Discord 成员），或一个**群聊**（见下）。成员从卡片头部的内联选择器添加，个人身份和群聊候选在同一个下拉里分区展示。每一行有两个互相独立的权限：
...
工作区的**可对话**不等于渠道的**聊天**。工作区权限授权客户端与工作区 API；渠道聊天权限则通过 ACL 引擎授权 IM 入站消息。
...
1. **涉及哪一层？** IM 消息走渠道聊天 ACL；客户端和工作区 API 走工作区成员。

...
- 未选择时的默认值：Codex 是 `medium`，Claude Code 是 `high`。
- 选择按会话独立，和模型选择互不影响。
...
Bot 的交互设置里有 **默认 Agent** 下拉框——可以选 Memoh 本体，或任一已启用且配置完成的 ACP 智能体。只影响之后新建的一对一会话，已有会话和群聊 Discuss 不受影响。
